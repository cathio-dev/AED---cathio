#ifndef ARVORETRIE2_H
#define ARVORETRIE2_H
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#define LETRAS 26
...
tp_no *cria_no(char letra)
{
    ...
    if (no != NULL)
    {
        ...
        no->letra = letra;
        for (i = 0; i < LETRAS; i++)
            no->filhos[i] = NULL; // colocando todas as letras NULL
    }
    return no;
}
...
int remove_no_trie(tp_no **raiz, char *palavra)
{
    ...
    if (*palavra)
    {
        if (*raiz != NULL && (*raiz)->filhos[*palavra - 'a'] != NULL && remove_no_trie(&((*raiz)->filhos[*palavra - 'a']), palavra + 1) && (*raiz)->eh_folha == 0)
        {
            if (!tem_filho(*raiz))
            {
                free(*raiz);
...
void imprime_arv_trie(tp_no *raiz)
{
    ...
    for (i = 0; i < LETRAS; i++)
    {
        imprime_arv_trie(no->filhos[i]); // recursividade colocando cada posição do vetor para ser imprimida
    }
}
